...
<div class="px-6 md:px-16 py-10 bg-[#FFFFF0] min-h-screen">
    <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6 gap-4">
        <div>
            <h1 class="text-4xl font-extrabold text-[#800020] mb-2 md:mb-0">Users List</h1>
        ...
        <table class="w-full text-sm md:text-base table-auto">
            <thead class="bg-[#800020] text-[#D4AF37] font-semibold select-none">
                <tr>
                    ...
                    <th class="text-left px-6 py-3">Registered On</th>
            ...
            <tbody class="text-[#4B2E0A]">
                ...
                <tr class="border-t border-[#D4AF37]/40 hover:bg-[#f9f3e8] transition">
                    <td class="px-6 py-3 font-semibold">{{ $user->id }}</td>
                    <td class="px-6 py-3">
                        @php
                            $avatarSrc = null;
                            if ($user->photo) {
                                $avatarSrc = asset($user->photo); // from /public/ or /storage
                            } elseif ($user->profile_image) {
                                $avatarSrc = $user->profile_image; // social login image
                            }
                        @endphp
                    ...
                    <td class="px-6 py-3 truncate max-w-xs">{{ $user->email }}</td>
                    <td class="px-6 py-3">{{ $user->address ?? 'N/A' }}</td>
                ...
                @empty
                <tr>
                    <td colspan="8" class="text-center px-6 py-8 text-gray-500 italic select-none">No users found.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
