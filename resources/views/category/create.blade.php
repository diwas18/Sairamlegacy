@extends('layouts.app')
@section('content')
<h1 class="text-4xl font-extrabold text-blue-900">Create Categories</h1>
...
<form action="{{ route('category.store')}}" method="POST" class="mt-5">
    ...
    <div class="flex mt-4 justify-center gap-4">
        <input type="submit" value="Add Category" class="bg-blue-600 text-white px-5 py-3 rounded-lg
        cursor-pointer">
        <a href="{{ route('category.index') }}" class="bg-red-600 text-white px-10 py-3 rounded-lg">
            Exit </a>
    </div>
